...
drop table if exists tempQueries
drop table if exists tempQueriesCopy
	...
	,'select ' + STUFF((
			select ',' + case 
					...
					when DATA_TYPE = 'binary'
						...
						or DATA_TYPE = 'image'
						then char(39) +'\x'+ char(39) +' + substring(master.dbo.fn_varbintohexstr([' + COLUMN_NAME + ']), 3, len(master.dbo.fn_varbintohexstr(' + COLUMN_NAME + ')))'
					when DATA_TYPE = 'geography'
						then char(39) +'\x'+ char(39) +' + substring(master.dbo.fn_varbintohexstr([' + COLUMN_NAME + '].STAsBinary()), 3, len(master.dbo.fn_varbintohexstr(' + COLUMN_NAME + '.STAsBinary())))'
					when DATA_TYPE = 'xml'
						then @databaseName + '.dbo.GetStringFromXML([' + COLUMN_NAME + '])'
...
into tempQueries
from sys.tables t
where t.name <> 'tempQueries' and t.name <> 'sysdiagrams'
...
begin
	...
	select @sql = 'bcp "' + @query2 + '" queryout "' + @folderPath + replace( @table,'"','') + '.csv" -c -C 65001 -t~ -T -S' + @@servername
